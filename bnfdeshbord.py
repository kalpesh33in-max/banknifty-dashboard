...
# Wider range to ensure we can always find 5 ITM strikes
...
# No custom CSS is needed for a dark theme, as it's the default.
# The styling function will handle cell colors.
    ...
    try:
        async with websockets.connect(WSS_URL) as websocket:
            ...
            if not auth_response.get("Complete"):
                # No st.error here to keep UI clean, will be handled by return value
...
with col2:
    # The "selected_atm" is now only for coloring the ATM strike
    selected_atm = st.selectbox('Select ATM for Coloring', options=list(STRIKE_RANGE), index=list(STRIKE_RANGE).index(st.session_state.get('atm_strike', 60100)))
